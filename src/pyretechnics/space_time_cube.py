...
class SpaceTimeCube:
    ...
    def __init__(self, base, t, y, x):
        ...
        match np.ndim(base):
            ...
            case 1:
                t0 = len(base)
                t_repetitions = divide_evenly(t, t0)
                # Repeat (t0) -> (t)              [NOTE: Inefficient memory usage]
                repeated_array = maybe_repeat_array(base, (0, t_repetitions))
                # Expand (t) -> (t,1,1)
            ...
            case 2:
                (y0, x0) = np.shape(base)
                y_repetitions = divide_evenly(y, y0)
                x_repetitions = divide_evenly(x, x0)
