...
def spread_fire_one_timestep(space_time_cubes, output_matrices, frontier_cells, tracked_cells,
                             ...
                             use_wind_limit=True, max_length_to_width_ratio=None, max_cells_per_timestep=0.4,
                             buffer_width=3):
    """
    TODO: Add docstring
    NOTE:
    ...
    for cell_index in tracked_cells:
        ...
        fire_behavior         = burn_cell_toward_phi_gradient(space_time_cubes, space_time_coordinate,
    ...
    for cell_index in tracked_cells:
        ...
        fire_behavior_star    = burn_cell_toward_phi_gradient(space_time_cubes, space_time_coordinate,
...
def spread_fire_with_phi_field(space_time_cubes, output_matrices, cell_width, cell_height, band_duration, start_time,
    ...
    while(simulation_time < max_stop_time and len(tracked_cells) > 0):
        # Compute max_timestep based on the remaining time in the simulation
        max_timestep = max_stop_time - simulation_time

        # Spread fire one timestep
        results = spread_fire_one_timestep(space_time_cubes, output_matrices, frontier_cells, tracked_cells,
                                           cell_width, cell_height, band_duration, simulation_time, max_timestep, 
                                           use_wind_limit, max_length_to_width_ratio, max_cells_per_timestep,
                                           buffer_width)
        # Reset spread inputs
