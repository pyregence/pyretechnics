...
# FIXME: REVIEW Should I have kept this as sigma_y_scalar_ft? Is GridFire incorrect here?
...
def himoto_resolve_default_sigma_y_from_lognormal_params(mu_x, sigma_x):
    ...
    avg_deltax_m = exp(mu_x) * es2h
    return sigma_y_scalar_m * avg_deltax_m * (es2h + 1.0) * (es2h - 1.0) # meters
...
def is_spot_ignition(rand_gen, spot_ignition_probability):
    """
    Returns True if a real value drawn uniformly from the range [0,1) is <= spot_ignition_probability.
    """
    return rand_gen.uniform(0.0, 1.0) <= spot_ignition_probability
