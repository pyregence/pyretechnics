...
def calc_surface_fire_behavior_no_wind_no_slope(
    ...
    base_spread_rate: cy.float = ft_to_m(R0 * spread_rate_adjustment)
    base_fireline_intensity: cy.float = Btu_ft_s_to_kW_m(I_s * spread_rate_adjustment)
    max_effective_wind_speed: cy.float = ft_to_m(U_eff_max)
...
def calc_surface_fire_behavior_max(sfmin: FireBehaviorMin, midflame_wind_speed: cy.float, upwind_direction: cy.float,
    ...
    phi_W = get_phi_W(sfmin, vector_magnitude_3d(wind_vector_3d)) # |wind_vector_3d| = slope-aligned midflame wind speed
    ...
    if phi_E > 0.0:
        ...
        max_spread_direction = as_unit_vector_3d(slope_vector_3d)
    else:
