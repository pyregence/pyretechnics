...
\begin{align*}
  r &= \frac{\delta\phi_{\textrm{up}}}{\delta\phi_{\textrm{loc}}} \\
  B(r) &= \textrm{max}(0, \textrm{min}(2r, 1), \textrm{min}(r, 2))
\end{align*}
...
\begin{align*}
  \frac{\partial{\phi}}{\partial x} &\approx \frac{\phi_{\textrm{east}} - \phi_{\textrm{west}}}{\Delta x} \\
  \frac{\partial{\phi}}{\partial y} &\approx \frac{\phi_{\textrm{north}} - \phi_{\textrm{south}}}{\Delta y}
\end{align*}
...
\begin{align*}
  \delta\phi_{\textrm{loc}} &= \phi_{i+1,j} - \phi{i,j} \\
  \\
  \delta\phi_{\textrm{up}} &= \begin{cases}
                                \phi_{i,j} - \phi_{i-1,j} & \textrm{for} u_{x,i,j} \geq 0 \\
                                \phi_{i+2,j} - \phi_{i+1,j} & \textrm{for} u_{x,i,j} < 0
                              \end{cases} \\
  \\
  \delta\phi_{\textrm{east}} &= \begin{cases}
                                  \phi_{i,j} + \frac{1}{2}B(r)\delta\phi_{loc} & \textrm{for} u_{x,i,j} \geq 0 \\
                                  \phi_{i+1,j} - \frac{1}{2}B(r)\delta\phi_{loc} & \textrm{for} u_{x,i,j} < 0
                                \end{cases}
\end{align*}
...
\begin{align*}
  ...
  \\
  \delta\phi_{\textrm{up}} &= \begin{cases}
                                \phi_{i-2,j} - \phi_{i-1,j} & \textrm{for} u_{x,i,j} \geq 0 \\
                                \phi_{i,j} - \phi_{i+1,j} & \textrm{for} u_{x,i,j} < 0
                              \end{cases} \\
  \\
  \delta\phi_{\textrm{west}} &= \begin{cases}
                                  \phi_{i-1,j} - \frac{1}{2}B(r)\delta\phi_{loc} & \textrm{for} u_{x,i,j} \geq 0 \\
                                  \phi_{i,j} + \frac{1}{2}B(r)\delta\phi_{loc} & \textrm{for} u_{x,i,j} < 0
                                \end{cases}
\end{align*}
...
\begin{align*}
  \delta\phi_{\textrm{loc}} &= \phi_{i,j+1} - \phi{i,j} \\
  \\
  \delta\phi_{\textrm{up}} &= \begin{cases}
                                \phi_{i,j} - \phi_{i,j-1} & \textrm{for} u_{y,i,j} \geq 0 \\
                                \phi_{i,j+2} - \phi_{i,j+1} & \textrm{for} u_{y,i,j} < 0
                              \end{cases} \\
  \\
  \delta\phi_{\textrm{north}} &= \begin{cases}
                                   \phi_{i,j} + \frac{1}{2}B(r)\delta\phi_{loc} & \textrm{for} u_{y,i,j} \geq 0 \\
                                   \phi_{i,j+1} - \frac{1}{2}B(r)\delta\phi_{loc} & \textrm{for} u_{y,i,j} < 0
                                 \end{cases}
\end{align*}
...
\begin{align*}
  ...
  \\
  \delta\phi_{\textrm{up}} &= \begin{cases}
                                \phi_{i,j-2} - \phi_{i,j-1} & \textrm{for} u_{y,i,j} \geq 0 \\
                                \phi_{i,j} - \phi_{i,j+1} & \textrm{for} u_{y,i,j} < 0
                              \end{cases} \\
  \\
  \delta\phi_{\textrm{south}} &= \begin{cases}
                                   \phi_{i,j-1} - \frac{1}{2}B(r)\delta\phi_{loc} & \textrm{for} u_{y,i,j} \geq 0 \\
                                   \phi_{i,j} + \frac{1}{2}B(r)\delta\phi_{loc} & \textrm{for} u_{y,i,j} < 0
                                 \end{cases}
\end{align*}
