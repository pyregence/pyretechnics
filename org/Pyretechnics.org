...
- config_dict :: a dictionary of parameter names to their values with the following types:
  ...
  | Parameter Name               | Type  | Units                                            |
  |------------------------------+-------+--------------------------------------------------|
  | firebrands_per_cell          | int   | number of embers to cast from each spotting cell |
  | decay_constant               | float | TODO: find units range                           |
  | mean_distance                | float | TODO: find units range                           |
  | flin_exp                     | float | TODO: find units range                           |
  | ws_exp                       | float | TODO: find units range                           |
  | normalized_distance_variance | float | TODO: find units range                           |
  | delta_y_sigma                | float | TODO: find units range                           |
  |------------------------------+-------+--------------------------------------------------|
...
- input_layer_dict :: a dictionary of layer names to either 2-arity (2D) or 3-arity (3D) functions for accessing the numeric values at each =(y,x)= or =(z,y,x)= coordinate in the simulated space-time cube:
  ...
  | surface_spotting_percent             | float | percent [0-100]                                        | Yes       |     2 |
  | crown_spotting_percent               | float | percent [0-100]                                        | Yes       |     2 |
  | temperature                          | float | degrees Celsius                                        |           |     3 |
  | relative_humidity                    | float | percent [0-100+]                                       |           |     3 |
  | wind_speed                           | float | meters/second                                          |           |     3 |
  | wind_direction                       | float | degrees clockwise from North [0-360)                   |           |     3 |
...
- output_layer_dict :: an *optional* dictionary of layer names to 2D Numpy arrays with the following units to pre-populate the simulation output arrays:
  ...
  |---------------------------------+--------------------------+------------------------------------------------------------------------------+-----------|
  | time_of_arrival                 | 2D chunked float array   | minutes from the start time of the simulated space-time cube                 | Yes       |
  | max_surface_spread_rate         | 2D chunked float array   | meters/second                                                                | Yes       |
  | max_crown_spread_rate           | 2D chunked float array   | meters/second                                                                | Yes       |
  | perimeter_spread_rate           | 2D chunked float array   | meters/second                                                                | Yes       |
  | max_surface_spread_direction    | 2D chunked float array   | degrees clockwise from North                                                 | Yes       |
  | max_crown_spread_direction      | 2D chunked float array   | degrees clockwise from North                                                 | Yes       |
  | perimeter_spread_direction      | 2D chunked float array   | degrees clockwise from North                                                 | Yes       |
  | surface_eccentricity            | 2D chunked float array   | degrees clockwise from North                                                 | Yes       |
  | crown_eccentricity              | 2D chunked float array   | degrees clockwise from North                                                 | Yes       |
  | max_surface_fire_line_intensity | 2D chunked float array   | kilowatts/meter                                                              | Yes       |
  | max_crown_fire_line_intensity   | 2D chunked float array   | kilowatts/meter                                                              | Yes       |
  | perimeter_fire_line_intensity   | 2D chunked float array   | kilowatts/meter                                                              | Yes       |
  | fire_type                       | 2D chunked float array   | 0 = no_fire, 1 = surface_fire, 2 = passive_crown_fire, 3 = active_crown_fire | Yes       |
  | ember_count                     | 2D chunked float array   | number of embers landing in each cell                                        | Yes       |
  | ember_ignition                  | 2D chunked boolean array | True for cells ignited by embers, False otherwise                            | Yes       |
  |---------------------------------+--------------------------+------------------------------------------------------------------------------+-----------|
...
world_state = {
    ...
    output_layer_dict: {}, # Don't pre-populate the output layers
...
def grow_fire(world_state):
    ...
    4. Record the time-of-arrival in each cell as it spreads
    ...
    # RESUME HERE: [[file:~/code/sig-gis/gridfire/src/gridfire/fire_spread.clj::(defmulti ^:private run-fire-spread*]]
