...
toa_100_100  = layer_lookup["time_of_arrival"](100,100)
sdi_100_100  = layer_lookup["suppression_difficulty_index"](100,100)
...
# FIXME: stub
def burn_cells(config_dict, layer_dict, ignited_cells_set, outputs_list):
    """
    1. Validate inputs
       - Make sure that all required parameters are present in config_dict (like spec-checking config.edn)
         - simulation-shape (z,y,x)
         - cell-height
         - cell-width
         - RESUME HERE: [[file:~/code/sig-gis/gridfire/src/gridfire/core.clj::inputs/add-misc-params][add-misc-params]]
       ...
       - Check for entries in layer_dict for output layers; if present, copy/use as new output layers
       - Use time of arrival raster to identify the burn scar(s); if missing, we are simulating point ignitions
       - Specify 3D space-time points in ignited_cells_set
