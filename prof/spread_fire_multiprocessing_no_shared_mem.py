...
# NOTE: All local arrays will have dtype=np.float32 to avoid copying
#       into the child processes when converted to SpaceTimeCubes.
def make_local_array(shape, value):
...
def spread_one_fire(cube_shape, cube_resolution, ignited_cell,
                    start_time, max_duration, spot_config, cube_refresh_rates):
    #============================================================================================
    # Create a dictionary of arrays (using constant arrays for this example)
    #============================================================================================
    ...
    local_arrays = {
        "slope"                        : make_local_array(hi_res_grid_shape,   0.80), # rise/run
        "aspect"                       : make_local_array(hi_res_grid_shape, 225.00), # degrees clockwise from North
        "fuel_model"                   : make_local_array(hi_res_grid_shape, 185   ), # (TL5) high-load conifer litter
        "canopy_cover"                 : make_local_array(hi_res_grid_shape,   0.70), # 0-1
        "canopy_height"                : make_local_array(hi_res_grid_shape,  10.00), # m
        "canopy_base_height"           : make_local_array(hi_res_grid_shape,   0.50), # m
        "canopy_bulk_density"          : make_local_array(hi_res_grid_shape,   0.30), # kg/m^3
        "wind_speed_10m"               : make_local_array(lo_res_cube_shape,  10.00), # km/hr
        "upwind_direction"             : make_local_array(lo_res_cube_shape, 180.00), # degrees clockwise from North
        "fuel_moisture_dead_1hr"       : make_local_array(lo_res_cube_shape,   0.03), # kg moisture/kg ovendry weight
        "fuel_moisture_dead_10hr"      : make_local_array(lo_res_cube_shape,   0.04), # kg moisture/kg ovendry weight
        "fuel_moisture_dead_100hr"     : make_local_array(lo_res_cube_shape,   0.05), # kg moisture/kg ovendry weight
        "fuel_moisture_live_herbaceous": make_local_array(lo_res_cube_shape,   0.90), # kg moisture/kg ovendry weight
        "fuel_moisture_live_woody"     : make_local_array(lo_res_cube_shape,   0.60), # kg moisture/kg ovendry weight
        "foliar_moisture"              : make_local_array(lo_res_cube_shape,   0.70), # kg moisture/kg ovendry weight
        "temperature"                  : make_local_array(lo_res_cube_shape,  30.00), # degrees Celsius
        "fuel_spread_adjustment"       : make_local_array(hi_res_grid_shape,   1.00), # float >= 0.0 (Optional: default = 1.0)
        "weather_spread_adjustment"    : make_local_array(lo_res_cube_shape,   1.00), # float >= 0.0 (Optional: default = 1.0)
    }

    #============================================================================================
    # Create an input dictionary of SpaceTimeCubes from the arrays
    #============================================================================================

    space_time_cubes = {name: SpaceTimeCube(cube_shape, local_array)
